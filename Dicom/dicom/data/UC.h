...
namespace dicom::data {
	...
	class DICOM_EXPORT UC : public VR
	{
	public:
        ...
        UC();
        UC(const encoded_string& value);
        UC(encoded_string&& value);
        UC(const std::vector<encoded_string>& values);
        UC(std::initializer_list<encoded_string> values);
        UC(const UC& other);
        UC(UC&& other);
		virtual ~UC();

        UC& operator = (const UC& other);
        UC& operator = (UC&& other);

        //----------------------------------------------------------------------------------------------------

        [[nodiscard]] bool Empty() const override final { return m_value.Empty(); }
        [[nodiscard]] std::unique_ptr<VR> Copy() const override final { return std::make_unique<UC>(*this); }
    ...
    // STL ADL functions
    inline auto begin(const UC& x) { return x.ParsedBegin(); }
    inline auto end(const UC& x) { return x.ParsedEnd(); }
