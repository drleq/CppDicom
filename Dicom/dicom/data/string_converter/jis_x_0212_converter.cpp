...
namespace {
    ...
    // Data based on ftp://ftp.unicode.org/Public/MAPPINGS/OBSOLETE/EASTASIA/JIS/JIS0212.TXT
    //   - Covers entire JIS X 0212 to UTF16 conversion.
    //   - LUT tables grouped and indexed by JIS X 0212 (row,col) pairs, collapsing gaps. 0x22 is first, 0x6D is last.
    constexpr uint16_t s_0x22[21] = {
        // 0x22 (0x2F-0x39, 0x42-0x44, 0x6B-0x71)
        0x02D8, 0x02C7, 0x00B8, 0x02D9, 0x02DD, 0x00AF, 0x02DB, 0x02DA, 
        0x007E, 0x0384, 0x0385, 0x00A1, 0x00A6, 0x00BF, 0x00BA, 0x00AA, 
        0x00A9, 0x00AE, 0x2122, 0x00A4, 0x2116
    };
    constexpr uint16_t s_0x26[21] = {
        // 0x26 (0x61-0x65, 0x67-0x67, 0x69-0x6A, 0x6C-0x6C, 0x71-0x7C)
        0x0386, 0x0388, 0x0389, 0x038A, 0x03AA, 0x038C, 0x038E, 0x03AB, 
        0x038F, 0x03AC, 0x03AD, 0x03AE, 0x03AF, 0x03CA, 0x0390, 0x03CC, 
        0x03C2, 0x03CD, 0x03CB, 0x03B0, 0x03CE
    };
    constexpr uint16_t s_0x27[26] = {
        // 0x27 (0x42-0x4E, 0x72-0x7E)
        0x0402, 0x0403, 0x0404, 0x0405, 0x0406, 0x0407, 0x0408, 0x0409, 
        0x040A, 0x040B, 0x040C, 0x040E, 0x040F, 0x0452, 0x0453, 0x0454, 
        0x0455, 0x0456, 0x0457, 0x0458, 0x0459, 0x045A, 0x045B, 0x045C, 
        0x045E, 0x045F
    };
    constexpr uint16_t s_0x29[27] = {
        // 0x29 (0x21-0x22, 0x24-0x24, 0x26-0x26, 0x28-0x29, 0x2B-0x2D, 0x2F-0x30, 0x41-0x50)
        0x00C6, 0x0110, 0x0126, 0x0132, 0x0141, 0x013F, 0x014A, 0x00D8, 
        0x0152, 0x0166, 0x00DE, 0x00E6, 0x0111, 0x00F0, 0x0127, 0x0131, 
        0x0133, 0x0138, 0x0142, 0x0140, 0x0149, 0x014B, 0x00F8, 0x0153, 
        0x00DF, 0x0167, 0x00FE
    };
    constexpr uint16_t s_0x2A[86] = {
        // 0x2A (0x21-0x38, 0x3A-0x77)
    ...
    constexpr uint16_t s_0x2B[85] = {
        // 0x2B (0x21-0x3B, 0x3D-0x43, 0x45-0x77)
        ...
        0x01F5, 0x011D, 0x011F, 0x0121, 0x0125, 0x00ED, 0x00EC, 0x00EF, 
        0x00EE, 0x01D0, 0x012B, 0x012F, 0x0129, 0x0135, 0x0137, 0x013A, 
        ...
        0x00FF, 0x0177, 0x017A, 0x017E, 0x017C
    };
    constexpr uint16_t s_0x30_6C[5734] = {
        // 0x30 (0x21-0x7E)
...
namespace dicom::data::string_converter {
    ...
    bool jis_x_0212_to_utf8(const std::string_view& encoded_source, std::string& utf8_dest) {
        ...
        return detail::encoded_to_utf8(
            ...
            [](const uint8_t*& next_it, const uint8_t* end_it, char32_t& decoded) {
                ...
                // JIS X 0212 is a 16-bit encoding
                uint16_t c = *reinterpret_cast<const uint16_t*>(next_it);

                // JIS X 0212 to Unicode is non-trivial. We must use a lut. However, JIS X 0212 does not
                ...
                if (j1 < 0x22 || j1 > 0x6D) { return false; }
                if (j2 < 0x21 || j2 > 0x7E) { return false; }

                // First check the cases where 0x21 <= j2 <= 0x7E (entire row is contiguous)
                if (j1 >= 0x30 && j1 <= 0x6C) { decoded = s_0x30_6C[((j1 - 0x30) * 94) + j2_0]; }
                else {
                    // Check the minority cases.
                    switch (j1) {
                    case 0x22: // 0x22 (0x2F-0x39, 0x42-0x44, 0x6B-0x71)
                             if (j2 >= 0x2F && j2 <= 0x39) { decoded = s_0x22[j2 - 0x2F]; }
                        else if (j2 >= 0x42 && j2 <= 0x44) { decoded = s_0x22[j2 - 0x42 + 11]; }
                        else if (j2 >= 0x6B && j2 <= 0x71) { decoded = s_0x22[j2 - 0x6B + 14]; }
                        else { return false; }
                        break;

                    case 0x26: // 0x26 (0x61-0x65, 0x67-0x67, 0x69-0x6A, 0x6C-0x6C, 0x71-0x7C)
                             if (j2 >= 0x61 && j2 <= 0x65) { decoded = s_0x26[j2 - 0x61]; }
                        else if (j2 == 0x67 )              { decoded = s_0x26[j2 - 0x67 + 5]; }
                        else if (j2 >= 0x69 && j2 <= 0x6A) { decoded = s_0x26[j2 - 0x69 + 6]; }
                        else if (j2 == 0x6C)               { decoded = s_0x26[j2 - 0x6C + 8]; }
                        else if (j2 >= 0x71 && j2 <= 0x7C) { decoded = s_0x26[j2 - 0x71 + 9]; }
                        else { return false; }
                        break;

                    case 0x27: // 0x27 (0x42-0x4E, 0x72-0x7E)
                             if (j2 >= 0x42 && j2 <= 0x4E) { decoded = s_0x27[j2 - 0x42]; }
                        else if (j2 >= 0x72 && j2 <= 0x7E) { decoded = s_0x27[j2 - 0x72 + 13]; }
                        else { return false; }
                        break;

                    case 0x29: // 0x29 (0x21-0x22, 0x24-0x24, 0x26-0x26, 0x28-0x29, 0x2B-0x2D, 0x2F-0x30, 0x41-0x50)
                             if (j2 <= 0x22)               { decoded = s_0x29[j2_0]; }
                        else if (j2 == 0x24)               { decoded = s_0x29[j2 - 0x24 + 2]; }
                        else if (j2 == 0x26)               { decoded = s_0x29[j2 - 0x26 + 3]; }
                        else if (j2 >= 0x28 && j2 <= 0x29) { decoded = s_0x29[j2 - 0x28 + 4]; }
                        else if (j2 >= 0x2B && j2 <= 0x2D) { decoded = s_0x29[j2 - 0x2B + 6]; }
                        else if (j2 >= 0x2F && j2 <= 0x30) { decoded = s_0x29[j2 - 0x2F + 9]; }
                        else if (j2 >= 0x41 && j2 <= 0x50) { decoded = s_0x29[j2 - 0x41 + 11]; }
                        else { return false; }
                        break;

                    case 0x2A: // 0x2A (0x21-0x38, 0x3A-0x77)
                             if (j2 <= 0x38)               { decoded = s_0x2A[j2_0]; }
                        else if (j2 >= 0x3A && j2 <= 0x77) { decoded = s_0x2A[j2 - 0x3A + 24]; }
                        else { return false; }
                        break;

                    case 0x2B: // 0x2B (0x21-0x3B, 0x3D-0x43, 0x45-0x77)
                             if (j2 <= 0x3B)               { decoded = s_0x2B[j2_0]; }
                        else if (j2 >= 0x3D && j2 <= 0x43) { decoded = s_0x2B[j2 - 0x3D + 27]; }
                        else if (j2 >= 0x45 && j2 <= 0x77) { decoded = s_0x2B[j2 - 0x45 + 34]; }
                        else { return false; }
                        break;

                    case 0x6D: // 0x6D (0x21-0x63)
                        if (j2 <= 0x63) { decoded = s_0x6D[j2_0]; }
                        else { return false; }
                        break;
    ...
    bool utf8_to_jis_x_0212(const std::string_view& utf8_source, std::string& encoded_dest) {
        // All JIS X 0212 characters map to BMP, but may use three-byte UTF-8 sequences.
        ...
        return detail::utf8_to_8bit_encoded<2>(
            ...
            [](char32_t c, uint8_t*& next_it) {
                ...
                // 0x30 -> 0x6C (0x21-0x7E)
                auto it = detail::find_codepoint00(s_0x30_6C, c16);
                if (it != nullptr) {
                    auto dist = distance(begin(s_0x30_6C), it);
                    auto block = dist / 94;
                ...
                if (it != nullptr) {
                    ...
                    auto column = dist;
                         if (dist <= 10) { column = 0x2F + dist; }
                    else if (dist <= 13) { column = 0x42 + (dist - 11); }
                    else { column = 0x6B + (dist - 14); }
                ...
                if (it != nullptr) {
                    ...
                    auto column = dist;
                         if (dist <= 4) { column = 0x61 + dist; }
                    else if (dist == 5) { column = 0x67 + (dist - 5); }
                    else if (dist <= 7) { column = 0x69 + (dist - 6); }
                    else if (dist == 8) { column = 0x6C + (dist - 8); }
                    else { column = 0x71 + (dist - 9); }
                ...
                if (it != nullptr) {
                    ...
                    auto column = dist;
                    if (dist <= 12) { column = 0x42 + dist; }
                    else { column = 0x72 + (dist - 13); }
                ...
                if (it != nullptr) {
                    ...
                    auto column = dist;
                         if (dist <=  1) { column = 0x21 + dist; }
                    else if (dist ==  2) { column = 0x24; }
                    else if (dist ==  3) { column = 0x26; }
                    else if (dist <=  5) { column = 0x28 + (dist - 4); }
                    else if (dist <=  8) { column = 0x2B + (dist - 6); }
                    else if (dist <= 10) { column = 0x2F + (dist - 9); }
                    else { column = 0x41 + (dist - 11); }
                    *dest_ptr = make_row_column(0x29, column);
                    return true;
                }
                ...
                if (it != nullptr) {
                    ...
                    else { column = 0x3A + (dist - 24); }
                    *dest_ptr = make_row_column(0x2A, column);
                    return true;
                }
                ...
                if (it != nullptr) {
                    ...
                    auto column = dist;
                         if (dist <= 26) { column = 0x21 + dist; }
                    else if (dist <= 33) { column = 0x3D + (dist - 27); }
                    else { column = 0x45 + (dist - 34); }
                    *dest_ptr = make_row_column(0x2B, column);
                    return true;
                }

                // 0x6D (0x21-0x63)
                it = detail::find_codepoint00(s_0x6D, c16);
                if (it != nullptr) {
                    auto dist = distance(begin(s_0x6D), it);
                    *dest_ptr = make_row_column(0x6D, 0x21 + dist);
                    return true;
                }
