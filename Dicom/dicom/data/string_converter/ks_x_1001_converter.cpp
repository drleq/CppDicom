...
namespace {
    // KS X 1001 -> UTF-16 conversion table.
    // Data based on ftp://ftp.unicode.org/Public/MAPPINGS/OBSOLETE/EASTASIA/KSC/KSX1001.TXT
    //   - Covers entire KS X 1001 to UTF-16 conversion.
    //   - LUT tables grouped and indexed by KS X 1001 (row,col) pairs, collapsing gaps. 0x21 is first, 0x7D is last.
    ...
    constexpr uint16_t s_0x23_24[2 * 94] = {
        // 0x23 (0x21-0x7E)
    ...
    constexpr uint16_t s_0x30_48[26 * 94] = {
        // 0x30 (0x21-0x7E)
    ...
    constexpr uint16_t s_0x4A_7D[52 * 94] = {
        // 0x4A (0x21-0x7E)
    ...
    [[nodiscard]] uint16_t make_row_column(T1 j1, T2 j2) {
        return (uint16_t(uint8_t(j2) + 0x80) << 8) | uint16_t(uint8_t(j1) + 0x80);
    }
}

namespace dicom::data::string_converter {

    bool ks_x_1001_to_utf8(const std::string_view& encoded_source, std::string& utf8_dest) {
        // All KS X 1001 characters map to BMP, but may use three-byte UTF-8 sequences.
        return detail::encoded_to_utf8(
            ...
            [](const uint8_t*& next_it, const uint8_t* end_it, char32_t& decoded) {
                ...
                // KS X 1001 to Unicode is non-trivial. We must use a lut. However, KS X 1001 does not
                // easily convert to a contiguous range either. We split the lut by row and special case any
                // gaps in the cells.
                uint8_t j1 = static_cast<uint8_t>((c & 0xFF) - 0x80);
                uint8_t j2 = static_cast<uint8_t>((c >> 8) - 0x80);
                uint8_t j2_0 = j2 - 0x21;

                if (j1 < 0x21 || j1 > 0x7D) { return false; }
                ...
                else if (j1 >= 0x23 && j1 <= 0x24) { decoded = s_0x23_24[((j1 - 0x23) * 94) + j2_0]; }
                else if (j1 == 0x29)               { decoded = s_0x29[j2_0]; }
                else if (j1 >= 0x30 && j1 <= 0x48) { decoded = s_0x30_48[((j1 - 0x30) * 94) + j2_0]; }
                else if (j1 >= 0x4A && j1 <= 0x7D) { decoded = s_0x4A_7D[((j1 - 0x4A) * 94) + j2_0]; }
                else {
                    // Check the minority cases.
                    switch (j1) {
                    case 0x22: // 0x22 (0x21-0x65)
                        if (j2 <= 0x65) { decoded = s_0x22[j2_0]; }
                        else { return false; }
                        break;

                    case 0x25: // 0x25 (0x21-0x2A, 0x30-0x39, 0x41-0x58, 0x61-0x78)
                             if (j2 <= 0x2A)               { decoded = s_0x25[j2_0]; }
                        else if (j2 >= 0x30 && j2 <= 0x39) { decoded = s_0x25[j2 - 0x30 + 10]; }
                        else if (j2 >= 0x41 && j2 <= 0x58) { decoded = s_0x25[j2 - 0x41 + 20]; }
                        else if (j2 >= 0x61 && j2 <= 0x78) { decoded = s_0x25[j2 - 0x61 + 44]; }
                        else { return false; }
                        break;

                    case 0x26: // 0x26 (0x21-0x64)
                        if (j2 <= 0x64) { decoded = s_0x26[j2_0]; }
                        else { return false; }
                        break;

                    case 0x27: // 0x27 (0x21-0x6F)
                        if (j2 <= 0x6F) { decoded = s_0x27[j2_0]; }
                        else { return false; }
                        break;

                    case 0x28: // 0x28 (0x21-0x24, 0x26-0x26, 0x28-0x2F, 0x31-0x7E)
                             if (j2 <= 0x24)               { decoded = s_0x28[j2_0]; }
                        else if (j2 == 0x26)               { decoded = s_0x28[j2 - 0x26 + 4]; }
                        else if (j2 >= 0x28 && j2 <= 0x2F) { decoded = s_0x28[j2 - 0x28 + 5]; }
                        else if (j2 >= 0x31)               { decoded = s_0x28[j2 - 0x31 + 13]; }
                        else { return false; }
                        break;

                    case 0x2A: // 0x2A (0x21-0x73)
                        if (j2 <= 0x73) { decoded = s_0x2A[j2_0]; }
                        else { return false; }
                        break;

                    case 0x2B: // 0x2B (0x21-0x76)
                        if (j2 <= 0x76) { decoded = s_0x2B[j2_0]; }
                        else { return false; }
                        break;

                    case 0x2C: // 0x2C (0x21-0x41, 0x51-0x71)
                             if (j2 <= 0x41) { decoded = s_0x2C[j2_0]; }
                        else if (j2 >= 0x51 && j2 <= 0x71) { decoded = s_0x2C[j2 - 0x51 + 33]; }
                        else { return false; }
                        break;
    ...
    bool utf8_to_ks_x_1001(const std::string_view& utf8_source, std::string& encoded_dest) {
        // All KS X 1001 characters map to BMP, but may use three-byte UTF-8 sequences.
        ...
        return detail::utf8_to_8bit_encoded<2>(
            ...
            [](char32_t c, uint8_t*& next_it) {
                ...
                if (it != nullptr) {
                    ...
                    auto column = dist;
                         if (dist <=  9) { column = 0x21 + dist; }
                    else if (dist <= 19) { column = 0x30 + (dist - 10); }
                    else if (dist <= 43) { column = 0x41 + (dist - 20); }
                    else { column = 0x61 + (dist - 44); }
                    *dest_ptr = make_row_column(0x25, column);
                    return true;
                }
                ...
                if (it != nullptr) {
                    ...
                    auto column = dist;
                         if (dist <=  3) { column = 0x21 + dist; }
                    else if (dist ==  4) { column = 0x26; }
                    else if (dist <= 12) { column = 0x28 + (dist - 5); }
                    else { column = 0x31 + (dist - 13); }
                    *dest_ptr = make_row_column(0x28, column);
                    return true;
                }
                ...
                if (it != nullptr) {
                    ...
                    *dest_ptr = make_row_column(0x2C, column);
                    return true;
                }
