...
namespace {
    // JIS X 0208 -> UTF-16 conversion table.
    // Data based on Microsoft CP932: ftp://ftp.unicode.org/Public/MAPPINGS/OBSOLETE/EASTASIA/JIS/JIS0208.TXT
    //   - CP932 covers Shift JIS, which is a superset of JIS X 0208.
    //   - This LUT only covers the JIS X 0208 characters in CP932.
    //   - CP932 omits certain assigned ranges.
    //   - LUT tables grouped and indexed by JIS X 0208 (row,col) pairs, collapsing gaps. 0x21 is first, 0x73 is last.
    constexpr uint16_t s_0x21[94] = {
        // 0x21 (0x21-0x7E)
    ...
    constexpr uint16_t s_0x22[53] = {
        // 0x22 (0x21-0x2E, 0x3A-0x41, 0x4A-0x50, 0x5C-0x6A, 0x72-0x79, 0x7E)
        0x25C6, 0x25A1, 0x25A0, 0x25B3, 0x25B2, 0x25BD, 0x25BC, 0x203B, 
        0x3012, 0x2192, 0x2190, 0x2191, 0x2193, 0x3013, 0x2208, 0x220B, 
        0x2286, 0x2287, 0x2282, 0x2283, 0x222A, 0x2229, 0x2227, 0x2228, 
        0xFFE2, 0x21D2, 0x21D4, 0x2200, 0x2203, 0x2220, 0x22A5, 0x2312, 
        0x2202, 0x2207, 0x2261, 0x2252, 0x226A, 0x226B, 0x221A, 0x223D, 
        0x221D, 0x2235, 0x222B, 0x222C, 0x212B, 0x2030, 0x266F, 0x266D, 
        0x266A, 0x2020, 0x2021, 0x00B6, 0x25EF
    };
    constexpr uint16_t s_0x23[62] = {
        // 0x23 (0x30-0x39, 0x41-0x5A, 0x61-0x7A)
    ...
    constexpr uint16_t s_0x26[48] = {
        // 0x26 (0x21-0x38, 0x41-0x58)
    ...
    constexpr uint16_t s_0x27[66] = {
        // 0x27 (0x21-0x41, 0x51-0x71)
    ...
    constexpr uint16_t s_0x2D[83] = {
        // 0x2D (0x21-0x3E, 0x40-0x56, 0x5F-0x7C)
        ...
        0x339E, 0x338E, 0x338F, 0x33C4, 0x33A1, 0x337B, 0x301D, 0x301F, 
        ...
        0x2235, 0x2229, 0x222A
    };
    constexpr uint16_t s_0x30_4E[2914] = {
        // 0x30 (0x21-0x7E)
    ...
    constexpr uint16_t s_0x50_73[3384] = {
        // 0x50 (0x21-0x7E)
    ...
    [[nodiscard]] uint16_t make_row_column(T1 j1, T2 j2) {
        return (uint16_t(uint8_t(j2)) << 8) | uint16_t(uint8_t(j1));
    }
}

namespace dicom::data::string_converter {

    bool jis_x_0208_to_utf8(const std::string_view& encoded_source, std::string& utf8_dest) {
        // All JIS X 0208 characters map to BMP, but may use three-byte UTF-8 sequences.
        return detail::encoded_to_utf8(
            ...
            [](const uint8_t*& next_it, const uint8_t* end_it, char32_t& decoded) {
                ...
                // JIS X 0208 is a 16-bit encoding
                uint16_t c = *reinterpret_cast<const uint16_t*>(next_it);

                // JIS X 0208 to Unicode is non-trivial. We must use a lut. However, JIS X 0208 does not
                // easily convert to a contiguous range either. We split the lut by row and special case any
                // gaps in the cells.
                uint8_t j1 = static_cast<uint8_t>(c & 0xFF);
                uint8_t j2 = static_cast<uint8_t>(c >> 8);
                uint8_t j2_0 = j2 - 0x21;

                if (j1 < 0x21 || j1 > 0x74) { return false; }
                if (j2 < 0x21 || j2 > 0x7E) { return false; }

                // First check the cases where 0x21 <= j2 <= 0x7E (entire row is contiguous)
                     if (j1 == 0x21) { decoded = s_0x21[j2_0]; }
                else if (j1 >= 0x30 && j1 <= 0x4E) { decoded = s_0x30_4E[((j1 - 0x30) * 94) + j2_0]; }
                else if (j1 >= 0x50 && j1 <= 0x73) { decoded = s_0x50_73[((j1 - 0x50) * 94) + j2_0]; }
                else {
                    // Check the minority cases.
                    switch (j1) {
                    case 0x22: // 0x22 (0x21-0x2E, 0x3A-0x41, 0x4A-0x50, 0x5C-0x6A, 0x72-0x79, 0x7E)
                             if (j2 <= 0x2E) { decoded = s_0x22[j2_0]; }
                        else if (j2 >= 0x3A && j2 <= 0x41) { decoded = s_0x22[j2 - 0x3A + 14]; }
                        else if (j2 >= 0x4A && j2 <= 0x50) { decoded = s_0x22[j2 - 0x4A + 22]; }
                        else if (j2 >= 0x5C && j2 <= 0x6A) { decoded = s_0x22[j2 - 0x5C + 29]; }
                        else if (j2 >= 0x72 && j2 <= 0x79) { decoded = s_0x22[j2 - 0x72 + 44]; }
                        else if (j2 == 0x7E) { decoded = s_0x22[52]; }
                        else { return false; }
                        break;

                    case 0x23: // 0x23 (0x30-0x39, 0x41-0x5A, 0x61-0x7A)
                             if (j2 >= 0x30 && j2 <= 0x39) { decoded = s_0x23[j2 - 0x30]; }
                        else if (j2 >= 0x41 && j2 <= 0x5A) { decoded = s_0x23[j2 - 0x41 + 10]; }
                        else if (j2 >= 0x61 && j2 <= 0x7A) { decoded = s_0x23[j2 - 0x61 + 36]; }
                        else { return false; }
                        break;

                    case 0x24: // 0x24 (0x21-0x73)
                        if (j2 <= 0x73) { decoded = s_0x24[j2_0]; }
                        else { return false; }
                        break;

                    case 0x25: // 0x25 (0x21-0x76)
                        if (j2 <= 0x76) { decoded = s_0x25[j2_0]; }
                        else { return false; }
                        break;

                    case 0x26: // 0x26 (0x21-0x38, 0x41-0x58)
                             if (j2 <= 0x38) { decoded = s_0x26[j2_0]; }
                        else if (j2 >= 0x41 && j2 <= 0x58) { decoded = s_0x26[j2 - 0x41 + 24]; }
                        else { return false; }
                        break;

                    case 0x27: // 0x27 (0x21-0x41, 0x51-0x71)
                             if (j2 <= 0x41) { decoded = s_0x27[j2_0]; }
                        else if (j2 >= 0x51 && j2 <= 0x71) { decoded = s_0x27[j2 - 0x51 + 33]; }
                        else { return false; }
                        break;

                    case 0x28: // 0x28 (0x21-0x40)
                        if (j2 <= 0x40) { decoded = s_0x28[j2_0]; }
                        else { return false; }
                        break;

                    case 0x2D: // 0x2D (0x21-0x3E, 0x40-0x56, 0x5F-0x7C)
                             if (j2 <= 0x3E) { decoded = s_0x2D[j2_0]; }
                        else if (j2 >= 0x40 && j2 <= 0x56) { decoded = s_0x2D[j2 - 0x40 + 30]; }
                        else if (j2 >= 0x5F && j2 <= 0x7C) { decoded = s_0x2D[j2 - 0x5F + 53]; }
                        else { return false; }
                        break;

                    case 0x4F: // 0x4F (0x21-0x53)
                        if (j2 <= 0x53) { decoded = s_0x4F[j2_0]; }
                        else { return false; }
                        break;

                    case 0x74: // 0x74 (0x21-0x26)
                        if (j2 <= 0x26) { decoded = s_0x74[j2_0]; }
                        else { return false; }
                        break;
    ...
    bool utf8_to_jis_x_0208(const std::string_view& utf8_source, std::string& encoded_dest) {
        // All JIS X 0208 characters map to BMP, but may use three-byte UTF-8 sequences.
        ...
        return detail::utf8_to_8bit_encoded<2>(
            ...
            [](char32_t c, uint8_t*& next_it) {
                // JIS X 0208 to Unicode is non-trivial. We must use a lut. However, JIS X 0208 does not
                // easily convert to a contiguous range either. We will have to search each lut and derive
                ...
                // 0x21 (0x21-0x7E)
                auto it = detail::find_codepoint00(s_0x21, c16);
                if (it != nullptr) {
                    *dest_ptr = make_row_column(0x21, 0x21 + distance(begin(s_0x21), it));
                    return true;
                }

                // 0x30 -> 0x4E (0x21-0x7E)
                it = detail::find_codepoint00(s_0x30_4E, c16);
                if (it != nullptr) {
                    auto dist = distance(begin(s_0x30_4E), it);
                    auto block = dist / 94;
                    *dest_ptr = make_row_column(
                        0x30 + block,
                        0x21 + (dist - (block * 94))
                    );
                    return true;
                ...
                // 0x50 -> 0x73 (0x21-0x7E)
                it = detail::find_codepoint00(s_0x50_73, c16);
                if (it != nullptr) {
                    auto dist = distance(begin(s_0x50_73), it);
                    auto block = dist / 94;
                    *dest_ptr = make_row_column(
                        0x50 + block,
                        0x21 + (dist - (block * 94))
                ...
                if (it != nullptr) {
                    ...
                    auto column = dist;
                         if (dist <= 13) { column = 0x21 + dist; }
                    else if (dist <= 21) { column = 0x3A + (dist - 14); }
                    else if (dist <= 28) { column = 0x4A + (dist - 22); }
                    else if (dist <= 43) { column = 0x5C + (dist - 29); }
                    else if (dist <= 51) { column = 0x72 + (dist - 44); }
                    else { column = 0x7E; }
                    *dest_ptr = make_row_column(0x22, column);
                    return true;
                }

                // 0x23 (0x30-0x39, 0x41-0x5A, 0x61-0x7A)
                it = detail::find_codepoint00(s_0x23, c16);
                if (it != nullptr) {
                    auto dist = distance(begin(s_0x23), it);
                    auto column = dist;
                         if (dist <=  9) { column = 0x30 + dist; }
                    else if (dist <= 35) { column = 0x41 + (dist - 10); }
                    else { column = 0x61 + (dist - 36); }
                    *dest_ptr = make_row_column(0x23, column);
                    return true;
                }

                // 0x24 (0x21-0x73)
                it = detail::find_codepoint00(s_0x24, c16);
                if (it != nullptr) {
                    auto dist = distance(begin(s_0x24), it);
                    *dest_ptr = make_row_column(0x24, 0x21 + dist);
                    return true;
                }
                ...
                if (it != nullptr) {
                    ...
                    *dest_ptr = make_row_column(0x25, 0x21 + dist);
                    return true;
                }
                ...
                if (it != nullptr) {
                    ...
                    auto column = dist;
                    if (dist <= 23) { column = 0x21 + dist; }
                    else { column = 0x41 + (dist - 24); }
                    *dest_ptr = make_row_column(0x26, column);
                    return true;
                }
                ...
                if (it != nullptr) {
                    ...
                    auto column = dist;
                    if (dist <= 32) { column = 0x21 + dist; }
                    else { column = 0x51 + (dist - 33); }
                    *dest_ptr = make_row_column(0x27, column);
                    return true;
                }
                ...
                if (it != nullptr) {
                    ...
                    *dest_ptr = make_row_column(0x28, 0x21 + dist);
                    return true;
                }

                // 0x2D (0x21-0x3E, 0x40-0x56, 0x5F-0x7C)
                it = detail::find_codepoint00(s_0x2D, c16);
                if (it != nullptr) {
                    auto dist = distance(begin(s_0x2D), it);
                    auto column = dist;
                         if (dist <= 29) { column = 0x21 + dist; }
                    else if (dist <= 52) { column = 0x40 + (dist - 30); }
                    else { column = 0x5F + (dist - 53); }
                    *dest_ptr = make_row_column(0x2D, column);
                    return true;
                }

                // 0x4F (0x21-0x53)
                it = detail::find_codepoint00(s_0x4F, c16);
                if (it != nullptr) {
                    auto dist = distance(begin(s_0x4F), it);
                    *dest_ptr = make_row_column(0x4F, 0x21 + dist);
                    return true;
                }

                // 0x74 (0x21-0x26)
                it = detail::find_codepoint00(s_0x74, c16);
                if (it != nullptr) {
                    auto dist = distance(begin(s_0x74), it);
                    *dest_ptr = make_row_column(0x74, 0x21 + dist);
                    return true;
                }
