...
namespace dicom_test::data::string_converter {
    ...
    BOOST_FIXTURE_TEST_SUITE(iso_iec_8859_1_converter_test, TestFixture)

    //------------------------------------------------------------------------------------------------------------

    BOOST_AUTO_TEST_CASE(ValidToUTF16) {
        BOOST_REQUIRE(m_mapping->CheckValidByteSequences(iso_iec_8859_1_to_utf16));
    }

    //------------------------------------------------------------------------------------------------------------

    BOOST_AUTO_TEST_CASE(InvalidToUTF16) {
        BOOST_REQUIRE(m_mapping->CheckInvalidByteSequences(iso_iec_8859_1_to_utf16));
    }

    //------------------------------------------------------------------------------------------------------------

    BOOST_AUTO_TEST_CASE(ValidFromUTF16) {
        BOOST_REQUIRE(m_mapping->CheckValidUnicodeValues(utf16_to_iso_iec_8859_1));
    }

    //------------------------------------------------------------------------------------------------------------

    BOOST_AUTO_TEST_CASE(InvalidFromUTF16) {
        BOOST_REQUIRE(m_mapping->CheckInvalidUnicodeValues(utf16_to_iso_iec_8859_1));
    }
